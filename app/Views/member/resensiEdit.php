...
<div class="container-fluid">
    <div class="row">
        <div class="col-8">
            <h2 class="my-3"><a href="/member/resensiIndex" class="btn btn-primary"><i class="fas fa-arrow-left"></i></a> Form Ubah Resensi</h2>
            <form action="/Member/update/<?= $resensi['id_resensi']; ?>" method="POST" enctype="multipart/form-data">
                <?= csrf_field(); ?>
                <input type="hidden" name="id_resensi" value="<?= $resensi['id_resensi']; ?>">

                <div class="row mb-3">
                    <label for="judul_resensi" class="col-sm-2 col-form-label">Judul</label>
                    <div class="col-sm-10">
                    ...
                    <div class="col-sm-10">
                        <select name="id" class="form-control" aria-label="Default select example">
                            ...
                            <?php foreach ($user as $u) : ?>
                                <option value="<?= $u->userid; ?>" <?= $resensi['id'] == $u->userid ? 'selected' : 'disabled' ?>><?= $u->fullname; ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
